...
# from django.contrib.auth.decorators import f
...
def dashboard(request):
    ...
    today_activities = Activity.objects.filter(date=today)
    today_calories_burned = today_activities.aggregate(
        Sum('calories_burned')
    )['calories_burned__sum'] or 0
    today_duration = today_activities.aggregate(
    ...
    today_nutrition = NutritionEntry.objects.filter(date=today)
    today_calories_consumed = today_nutrition.aggregate(
        Sum('calories')
    )['calories__sum'] or 0
    today_protein = today_nutrition.aggregate(
    ...
    user_goal = UserGoal.objects.first()
...
#     )
def nutrition_list(request):
    nutrition_entries = NutritionEntry.objects.all().order_by('-date')
    return render(
        request,
        'health/nutrition_list.html',
        {'nutrition_entries': nutrition_entries}
...
#     )
def nutrition_update(request, pk):
    nutrition = get_object_or_404(NutritionEntry, pk=pk)
