...
class NutritionEntryForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['food_name', 'calories', 'protein', 'carbs', 'fat', 'quantity', 'date', 'meal_type']
        widgets = {
            'date': forms.DateInput(attrs={'type': 'date'}),
        }
...
class UserGoalForm(forms.ModelForm):
    class Meta:
        model = UserGoal
        fields = ['goal_type', 'target_weight', 'target_calories_burn', 'target_calories_consume', 'target_protein', 'target_activity_days']
        widgets = {
